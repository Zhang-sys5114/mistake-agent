...
- **目录即插件**：build.rs 扫描一层子目录里存在 `mod.rs` 的文件夹即收录；`services.rs` 是公共契约文件，不是插件。
...
- `session`：`switch` 工具（工具入口与 Session scheduler 分离，调度器在 `src/kernel/agent/session.rs`）；
...
- 公共契约与句柄：src/kernel/plugin/services.rs；注册表：src/kernel/registry.rs。

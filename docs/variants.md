...
`practice::generate` 已按本文档实现（practice 插件 + 3 个内置模板 + diagram_spec 输出，与前端 geometry.js 渲染器同源协议）：三角形全等（几何图）、绝对值（代数）、一般现在时三单（填空）。当前为确定性模板生成（不依赖 LLM），分层难度 basic/variant/advanced 通过参数槽控制。

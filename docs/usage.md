...
- **连续历史**：同一会话内聊天记录完整保留（旧消息自动携带，不回退）；**模型不会自作主张换话题**，开新会话由用户决定、旧会话自动归档，侧栏`会话列表`（`设置` 下方）里仍可查看并切回全部记录。
- **错题本**：左侧导航「错题本」直接查看错题列表（学科/知识点过滤、题目/作答/参考答案/错因）；也可在聊天里问"错题本里有什么"。
- **会话列表**：应用侧栏里、`会话列表` 开关图标下方展开的列表（按最近活动倒序，名字由模型按首条消息生成、可重命名/删除），点「新对话」开新会话、点任意会话切回；聊天中可对 assistant 消息编辑/重新生成，用 < > 切换消息树分支（只在当前会话内）。

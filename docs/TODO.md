...
## Agent core 剥离为 so-lite-agent（M1/M2 已落地，M3-M5 未落地）

把通用 Agent 运行时（loop/工具注册/会话/模型 Provider 抽象/通用 RPC）剥离为独立 crate `so-lite-agent`，开箱即用（`cargo add` 即可开发新 Agent），内核/用户插件由使用方编写。完整计划见 [docs/plan/so-lite-agent.md](plan/so-lite-agent.md)，决策见 [ADR-0037](adr/0037-so-lite-agent-crate-extraction.md)。当前 M1/M2 已落地，M3-M5 待办。
...
未落地：
- M3 Provider 适配器（OpenAI 兼容/Anthropic 兼容/自定义端点 + `register_provider`）与 DeepSeek 真实 API 回合。
- M4 插件手册/参考模板迁移到新 crate；使用方写内核插件 + 用户插件注册跑通。
